...
def parse_pems_dataset(pems_data: np.ndarray):
    ...
    data_hourly = data.groupby(["sid", "day", "hour"], as_index= False)[["speed", "occupancy", "flow"]].mean()
    data_hourly['weekday'] = data_hourly['day'] % 7 # day of week, number coded
    data_weekday = data_hourly.groupby(["sid", "weekday", "hour"], as_index= False)[["speed", "occupancy", "flow"]].mean()
    # pivot data to bring all features on a single sensor in 1 row
    data_weekly_tl = data_weekday.pivot(index="sid", columns = ["weekday", "hour"], values=["speed", "occupancy", "flow"])
